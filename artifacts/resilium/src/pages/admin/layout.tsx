...
import {
  ...
  Users, KeyRound, Activity, FolderLock,
  Star, BarChart2, FileText, LayoutTemplate, Radar, Rocket,
  BookOpen, Code2, Layers, Palette, ChevronDown, Newspaper, TrendingUp, FlaskConical, Instagram, Linkedin,
} from "lucide-react";
...
class AdminErrorBoundary extends Component<{ children: ReactNode }, AdminErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) { console.error("[AdminErrorBoundary]", error, info.componentStack); }
  render() {
    if (this.state.error) {
      return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4 p-8 text-center">
          ...
          <p className="text-muted-foreground text-sm max-w-md">{this.state.error.message ?? "An unexpected error occurred in this section."}</p>
          <button className="text-sm text-primary hover:underline" onClick={() => { this.setState({ error: null }); window.location.reload(); }}>Reload page</button>
        </div>
      );
    }
...
export interface AdminLayoutProps {
  ...
  activeSection?: "dashboard" | "testimonials" | "mobile" | "gdpr" | "users" | "marketing" | "security" | "monitoring" | "documents" | "blog" | "growth" | "instagram" | "linkedin";
}

/** @deprecated Token is now stored in an httpOnly cookie — no longer readable from JS. */
export function getAdminToken(): string | null { return null; }
/** Returns empty headers — admin session is carried via httpOnly cookie automatically. */
export function adminAuthHeaders(): Record<string, string> { return {}; }
...
function invalidateSessionCache() { _sessionCache.authed = false; _sessionCache.expiresAt = 0; }
...
export function AdminLayout({ children, activeSection }: AdminLayoutProps) {
  ...
  useEffect(() => {
    if (_sessionCache.expiresAt > Date.now()) {
      if (_sessionCache.authed) { setAuthed(true); setChecking(false); }
    ...
    fetch("/api/admin/session", { credentials: "include" })
      ...
      .catch(() => { invalidateSessionCache(); navigate("/admin/login"); })
  ...
  const navItems = [
    { href: "/admin/dashboard",  label: "Analytics",            icon: LayoutDashboard, key: "dashboard" },
    { href: "/admin/users",      label: "Users",                icon: Users,            key: "users" },
    { href: "/admin/blog",       label: "Blog Posts",           icon: Newspaper,        key: "blog" },
    { href: "/admin/growth",     label: "Growth Hub",           icon: TrendingUp,       key: "growth" },
    { href: "/admin/gdpr",       label: "Privacy & Consent",    icon: Shield,           key: "gdpr" },
    { href: "/admin/monitoring", label: "Testing & Monitoring", icon: FlaskConical,     key: "monitoring" },
    ...
    { href: "/admin/marketing",  label: "Marketing",            icon: Activity,         key: "marketing" },
    { href: "/admin/instagram",  label: "Instagram Studio",     icon: Instagram,        key: "instagram" },
    { href: "/admin/linkedin",   label: "LinkedIn Studio",      icon: Linkedin,         key: "linkedin" },
  ];
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          <div>
            <span
              className={cn(
                "flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium cursor-pointer transition-colors",
                isDocuments ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground",
              )}
              onClick={() => { if (!isDocuments) navigate("/admin/documents"); setDocsOpen(o => !o); }}
            >
            ...
            {docsOpen && (
              <div className="mt-1 ml-3 pl-3 border-l border-gray-200 space-y-0.5">
                {DOC_ITEMS.map(({ key, label, icon: Icon }) => (
                  <button key={key} type="button" onClick={() => navigate(`/admin/documents?doc=${key}`)}
                    className={cn(
                      "flex items-center gap-2 w-full text-left px-2 py-1.5 rounded-md text-xs transition-colors",
                      activeDoc === key ? "text-primary font-semibold bg-primary/8" : "text-muted-foreground hover:text-foreground hover:bg-muted",
                    )}>
