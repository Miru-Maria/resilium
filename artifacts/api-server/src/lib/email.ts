...
export async function sendReassessmentReminder(opts: { email: string; firstName?: string | null; lastScore: number; daysSince: number }) {
  if (!opts.email) return;
  const name = opts.firstName ?? "there";
