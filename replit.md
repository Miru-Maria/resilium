...
- **Admin Dashboard**: Provides comprehensive analytics, GDPR management, consent logging, and AI UX testing capabilities. Access is protected by `ADMIN_USERNAME`/`ADMIN_PASSWORD` secrets. Admin session token is stored in an httpOnly cookie (`admin_sess`) — never accessible to JavaScript.
