...
- **Admin Analytics Dashboard**: `/admin/analytics` page. Shows KPI cards (total users, conversion rate, Pro count, assessments), 12-month signup/assessment line charts, average dimension score bar chart (per-dimension coloring), plan-distribution bar chart, and a free→Pro conversion funnel. Backed by `GET /api/admin/analytics/users` (requires `admin_token`).
- **Growth Automation** (`/admin/growth`): Four-section admin page for growth tools: (1) Drip enrollment sync — backfills users missing from `emailDripQueueTable`, marks past stages cancelled, schedules future stages; (2) Referral program — `referral_codes` + `referral_uses` DB tables, `GET /api/referral/my-code` (auto-creates on first call), `POST /api/referral/claim` (Clerk-auth, idempotent, blocks self-referral); frontend `ReferralCapture` component in `App.tsx` stores `?ref=CODE` from landing URL to localStorage and auto-claims after Clerk authentication; (3) AI blog auto-generation — Monday 08:00 UTC cron calls OpenAI to draft a blog post (JSON: title, excerpt, content, tags, cta), emails admin via `sendBlogDraftReadyEmail`; (4) Broadcast email campaigns — `broadcast_campaigns` DB table, four audience segments (`all`, `pro`, `free`, `free_assessed_14d`), background send with 80 ms inter-recipient delay, full status tracking. Weekly digest now includes a generic "Share Resilium" referral CTA.
