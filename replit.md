...
- **Error Monitoring**: In-memory error rate tracking + admin email digest alerts (cron-based). Sentry is integrated across all three artifacts: `@sentry/react` (web, `src/main.tsx`), `@sentry/react-native` (mobile, `app/_layout.tsx`), and `@sentry/node` (API, `src/instrument.ts` + Express error handler in `src/app.ts`). Sentry is enabled only in production (`enabled: import.meta.env.PROD` on web, `enabled: !__DEV__` on mobile). DSNs: web=`4511187286818896`, mobile=`4511187296256080`, api=`4511187305431120` (all under org `o4511187075923968`, ingest at `ingest.de.sentry.io`).
